...
public class AbWheelView extends View {
	...
	/** The current item. *///所选择的当前的项目
	private int currentItem = 0;

	// Widths
	/** The items width. */// 相应的宽度
	...
	/** The visible items. *///显示的项目数
	private int visibleItems = DEF_VISIBLE_ITEMS;

	// Item height
	/** The item height. */// 项目的高度
...
//		desired = Math.max(desired, getSuggestedMinimumHeight());
	...
	private int calculateLayoutWidth(int widthSize, int mode) {
		...
		if (maxLength > 0) {
			//一个字符宽度
			float textWidth = (int)MobileUtil.getStringWidth("0", labelPaint);
			//不算lable的宽度
		...
		if (label != null && label.length() > 0) {
			labelWidth = (int)MobileUtil.getStringWidth(label, labelPaint);
		}
	 ...
	 */
	public void setValueTextSize(int textSize,int screenWidth,int screenHeight) {
		this.valueTextSize = MobileUtil.resizeTextSize(screenWidth,screenHeight, textSize);
	 ...
	 */
	public void setLabelTextSize(int labelTextSize,int screenWidth,int screenHeight) {
		this.labelTextSize = MobileUtil.resizeTextSize(screenWidth,screenHeight, labelTextSize);
	}

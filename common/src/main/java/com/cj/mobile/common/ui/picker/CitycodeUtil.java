...
import com.cj.mobile.common.model.Cityinfo;
...
public class CitycodeUtil {
	...
	public ArrayList<String> getProvince(List<Cityinfo> provice) {
		...
		for (int i = 0; i < provice.size(); i++) {
			province_list.add(provice.get(i).getRegionname());
			province_list_code.add(provice.get(i).getRegionid());
		}
	...
	public ArrayList<String> getCity(
		...
		for (int i = 0; i < city.size(); i++) {
			city_list.add(city.get(i).getRegionname());
			city_list_code.add(city.get(i).getRegionid());
		}
		return city_list;
	...
	public ArrayList<String> getCouny(
		...
		for (int i = 0; i < couny.size(); i++) {
			couny_list.add(couny.get(i).getRegionname());
			couny_list_code.add(couny.get(i).getRegionid());
		}
		return couny_list;
